...
q13_SucheProduktprivateAngepasst$Frequenz <- factor(q13_SucheProduktprivateAngepasst$Frequenz, levels = levels)
...
),5,levels(q13_SucheProduktprivateAngepasst$Quelle), levels, c(1:6), TRUE)
...
q13_SucheProduktfirmAngepasst$Frequenz <- factor(q13_SucheProduktfirmAngepasst$Frequenz, levels = levels)
...
),5,levels(q13_SucheProduktfirmAngepasst$Quelle), levels, c(1:6), TRUE)
...
),5,levels(q14_VertrauenAngepasst$Quelle), levels, c(1:8), TRUE)

knitr::kable(
  q_frequenz, longtable = TRUE, booktabs = TRUE
)

q14_VertrauenJoined %>% ggplot(aes(x=Frequenz, fill= Quelle)) + geom_bar(position = "dodge2") + xlab("Quelle") + ylab("Anzahl") + facet_wrap(.~AlterText)  + coord_flip()

q14_VertrauenJoined %>% ggplot(aes(x=Frequenz, fill= Quelle)) + geom_bar(position = "dodge2") + xlab("Quelle") + ylab("Anzahl") + facet_wrap(.~GeschlechtText)  + coord_flip()
...
levels = c("Interesse an einem Produkt / einer Dienstleistung", "Information über Aktionen", "Infromationen über neue Angebote","Inhalt der Informationen","Informationen über neue Trends", "Informationen über das unternehmen")
...
q_frequenz <- getfrequenz(subset(q15_EmailNewsletterprivat, select = columnNames1),1,levels(q15_EmailNewsletterprivatAngepasst$Grund), levels, c(1:6), TRUE)
...
q_frequenz <- getfrequenz(subset(q15_EmailNewsletterfirm, select = columnNames1),1,levels(q15_EmailNewsletterprivatAngepasst$Grund), levels, c(1:6), TRUE)
...
q18_QuellenSucheAngepasst$Frequenz <- factor(q18_QuellenSucheAngepasst$Frequenz, levels = levels)
...
),5,levels(q18_QuellenSucheAngepasst$Quelle), levels, c(1:11), TRUE)
...
q19_QuellenKaufAngepasst$Frequenz <- factor(q19_QuellenKaufAngepasst$Frequenz, levels = levels)
...
),2,levels(q19_QuellenKaufAngepasst$Quelle), levels, c(1:10), TRUE)
...
q21_MediumZukunftAngepasst <- q21_MediumZukunft  %>% subset(select =c(Nummer, Medium)) %>% 
  mutate(
  Medium = case_when(
    Medium == 1 ~ levels[1], 
    Medium == 2 ~ levels[2], 
